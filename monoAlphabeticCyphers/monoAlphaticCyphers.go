...
func Playfair(text, codeWord string ) string {
  ...
  for i := 1; i < len(newText); i+=2 {
    ...
    var newLetter1 string
    var newLetter2 string
    if pos1[1] == pos2[1] {
      // zelfde rij:
      // kolom+1
      newLetter1 = square.GetRune([2]int{pos1[0],(pos1[1]+1)%5})
      newLetter2 = square.GetRune([2]int{pos2[0],(pos2[1]+1)%5})
    } else {
      if pos1[1] == pos[1] {
        // zelfde kolom: rij+1
        newLetter1 = square.GetRune([2]int{(pos1[0]+1)%5,pos1[1]})
        newLetter2 = square.GetRune([2]int{(pos2[0]+1)%5,pos2[1]})
      } else {
        // andere rij en kolom
        // neem hoekpunten van de rechthoek
        newLetter1 = square.GetRune([2]int{pos1[0],pos2[1]})
        newLetter2 = square.GetRune([2]int{pos2[0],pos1[1]})
      }
    }
